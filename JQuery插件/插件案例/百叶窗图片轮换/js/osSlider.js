...
function osSlider(objs){
    ...
    that.pNode = $(that.objs.pNode); //pNode轮播的容器div
    that.cNodes = that.pNode.find(that.objs.cNode); //cNodes轮播的子节点对象集合(图片)
    that.cNodeNums = that.cNodes.length; //轮播图片的数量
    that.nowNodeKey = 0; //初始第一次默认显示节点为第一个
    that.width = that.cNodes.find('img').width();//轮播容器的宽度
    that.height = that.cNodes.find('img').height();//轮播容器的高度
    ...
    if (!that.objs.autoPlay) {//添加默认自动播放
        that.objs.autoPlay = false;
    }
    that.init = function() {//轮播的初始化
        that.pNode.addClass('osSlider-main');
        that.pNode.css({//轮播容器的大小控制 
            'width':that.width,//得到的是img的宽度
            'height':that.height,
            'overflow':'hidden',
            'position':'relative'
        });
        ...
        $toggleBtn.appendTo(that.pNode);//将切换按钮添加到轮播容器对象
        //为切换按钮绑定事件
        ...
        that.cNodes.each(function(index, el) {//that.cNodes得到容器中的li集合，对每个li元素进行遍历，添加前后顺序，有多少个图片就创建多少个高亮节点  index取0,1,2,3   el == this
            ...
            //当前的$(this)域指的是that.cNodes集合
            $(this).css({//为每一个li添加样式和顺序
                'width':that.width + 'px',
                'height':that.height + 'px',
                'overflow':'hidden',
                'position':'absolute',//div设了relative
                'top':'0px',
                'left':'0px',
                'z-index':indexNum// 图片根据z-index排序
            });
        });
        //为高亮导航节点绑定事件
     ...
     * 
     */
    that.sliderNavToggle = function(tid,nid) {//nid = that.nowNodeKey;
        $('.slider-nav li').each(function(index, el) {
            if (index==tid||index==nid) {
                $(this).toggleClass('active');//切换样式
            }
        });
    }
    /**
     * 切换效果指令函数 
     *  command 'prev'|'next'  字符串类型
     *  tid 下一个要切换的tid  数字类型
     ...
     * {Number} tid 下一个要切换的tid
     */
    that.moveSwitch = function(mid,command,tid) {
        nid = that.nowNodeKey;
        that.moveFlag = false;//添加是否进入下一个状态
        that.speedNum = 0;//自动轮播的计数
    ...
    that.gridTop = function(tid,showNum) {
        ...
        for (var i = 0; i < 12; i++) {//利用循环 创建出栅格节点
            ...
            $cvNode.find('*').first().css({//每次的第一个格珊距左的距离都不一样
                'display':'block',
